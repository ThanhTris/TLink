...
import { getPostsByCategory, countPostsSaved } from "../api/post";
...
export function toDate(mysqlDate: string): Date {
  const utcString = mysqlDate + "Z";
...
const Saved: React.FC = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        if (!user.id) return;
        const res = await countPostsSaved(user.id);
  ...
  // Lấy bài viết đã lưu theo trang
  const fetchPosts = async (page: number) => {
    try {
      ...
      // Gọi API lấy bài viết đã lưu (FE truyền "/saved")
  ...
  return (
    <div className="px-16 py-8">
      <ContentHeader title="Đã lưu" onCreate={() => setShowCreate(true)} />
      {showCreate && (
