...
import { getPostsByCategory, countPostsByParentTag } from "../../api/post";
...
const Career: React.FC = () => {
  ...
  // Lấy tổng số bài viết theo parent tag "career"
  useEffect(() => {
    let mounted = true;
    (async () => {
      try {
        const res = await countPostsByParentTag("career");
  ...
  const fetchPosts = async (page: number) => {
    try {
      ...
      // Gọi API lấy bài viết theo parent tag career (FE truyền "/career")
  ...
  return (
    <div className="px-16 py-8">
      <ContentHeader title="Tuyển dụng & nghề nghiệp" onCreate={() => setShowCreate(true)} />
      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20">
          <div className="w-full">
            <CreatePost
              ...
              initialTagParent="Tuyển dụng & nghề nghiệp"
            />
          </div>
        </div>
      )}
